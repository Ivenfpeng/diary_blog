...
const editHeading = computed(() =>
  editing.value?.kind === 'categories' ? 'Edit category' : 'Edit tag',
)
const taxonomyGroups = computed(() => [
  { type: 'categories' as const, label: 'Categories', values: categories.value },
  { type: 'tags' as const, label: 'Tags', values: tags.value },
])
...
async function create(): Promise<void> {
  try {
    const response = await apiRequest<Record<string, Taxonomy>>(
      `/api/admin/${kind.value}`,
      {
        method: 'POST',
...
async function saveEdit(): Promise<void> {
  ...
  try {
    const response = await apiRequest<Record<string, Taxonomy>>(
      `/api/admin/${current.kind}/${current.id}`,
      {
        method: 'PUT',
        body: { name: current.name, slug: current.slug },
      },
    )
...
async function confirmDelete(): Promise<void> {
  ...
  try {
    await apiRequest<void>(`/api/admin/${current.kind}/${current.id}`, {
      method: 'DELETE',
    })
    collection(current.kind).value = collection(current.kind).value.filter(
...
<template>
  <section class="taxonomy-view" aria-labelledby="taxonomy-heading">
        ...
        <div v-for="item in group.values" :key="item.id" class="taxonomy-row">
          ...
          <div class="row-actions">
            <button
              type="button"
              class="text-button"
              :data-testid="`edit-${group.type}-${item.id}`"
              @click="startEdit(group.type, item)"
            >
              Edit
            </button>
            <button
              ...
              :data-testid="`delete-${group.type}-${item.id}`"
              @click="deleting = { kind: group.type, ...item }"
            >
              Delete
            </button>
          </div>
    ...
    <section
      ...
      role="dialog"
      aria-modal="true"
      :aria-label="editHeading"
    >
      <h2>{{ editHeading }}</h2>

      <label>
        <span>Name</span>
        <input id="taxonomy-edit-name" v-model="editing.name" />
      </label>
      ...
      <div class="dialog-actions">
        ...
        <button
          ...
          data-testid="save-taxonomy-edit"
          @click="saveEdit"
        >
          Save changes
        </button>
    ...
    <section
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Confirm taxonomy deletion"
    >
      ...
      <div class="dialog-actions">
        ...
        <button
          type="button"
          class="danger-button"
          data-testid="confirm-taxonomy-delete"
          @click="confirmDelete"
        >
          Delete
        </button>
...
.dialog {
  position: fixed;
  inset: 20% max(16px, calc(50% - 220px)) auto;
  z-index: 2;
  display: grid;
  gap: 12px;
  padding: 24px;
