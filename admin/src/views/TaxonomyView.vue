...
<template>
  <section class="taxonomy-view" aria-labelledby="taxonomy-heading">
    ...
    <div class="lists">
      <section v-for="group in [{ type: 'categories' as const, label: 'Categories', values: categories }, { type: 'tags' as const, label: 'Tags', values: tags }]" :key="group.type">
        ...
        <div v-for="item in group.values" :key="item.id" class="taxonomy-row">
          ...
          <span>
            <button type="button" class="text-button" :data-testid="`edit-${group.type}-${item.id}`" @click="startEdit(group.type, item)">Edit</button>
            <button type="button" class="text-button danger" :data-testid="`delete-${group.type}-${item.id}`" @click="deleting = { kind: group.type, ...item }">Delete</button>
          </span>
        </div>
      </section>
    </div>
    <section v-if="editing" class="dialog" role="dialog" aria-modal="true" :aria-label="editHeading">
      <h2>{{ editHeading }}</h2>
      <label>Name <input id="taxonomy-edit-name" v-model="editing.name" /></label>
      <label>Slug <input id="taxonomy-edit-slug" v-model="editing.slug" /></label>
...
.secondary-button { border: 1px solid #9eaea5; background: #fff; }
.danger-button { border: 1px solid #a33a32; background: #a33a32; color: #fff; }
