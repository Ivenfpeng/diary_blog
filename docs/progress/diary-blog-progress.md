...
当前阶段：M2 Public Product

总体状态：进行中

最后登记：2026-09-04 10:20 CST
...
| M2 Public Product | Task 6-7 | 进行中 | 公开页面、搜索、RSS、Sitemap 和 SEO 可用 |
| M3 Authoring Product | Task 8-12 | 未开始 | 登录、管理 API、Vue 写作后台和媒体管理可用 |
...
| 7 | 搜索、RSS、Sitemap、SEO | 进行中 | `41d236e`, `7646a27` | 首轮实现通过；独立审查正在修复带转义路径前缀的 URL 重复编码 | 修复完成后再次专项审查 |
| 8 | 管理员密码、会话与 CSRF | 未开始 | - | - | - |
| 9 | 管理文章 API | 未开始 | - | - | - |
| 10 | Vue 后台框架与登录 | 未开始 | - | - | - |
...
| R-005 | 中 | 修复中 | 配置 URL 含转义路径前缀时 RSS/Sitemap 可能重复编码 | Task 7 修复轮统一 URL Path/RawPath 处理并加入回归测试 |
...
1. 完成 Task 7 URL 单次编码修复并通过专项复审。
2. 更新 Task 7 最终提交号和验证证据，将 M2 标记完成。
3. 自动进入 Task 8：管理员密码、会话、Cookie、CSRF 与登录限流。
4. 后续每个 Task 在实现提交和独立审查后同步更新本总控文档。
